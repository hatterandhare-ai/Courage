...
The pledge view polls `GET /api/pledge-state` instead — a lighter read (commitments/settings/pledgeBackground only) since it never needs the vote list, and it's the screen open on the most devices at once (every attendee's own phone). Both `/api/state` and `/api/pledge-state` are edge-cached for ~2s (`caches.default`), so many devices polling in the same window share one KV round-trip instead of each triggering their own — see "KV read volume" below.

## Known limitations

- **KV read volume**: earlier versions read every vote with a separate KV `get()` on every single poll (`votes + 4` reads per `/api/state` call), which at a well-attended event with the display/admin polling every 1-2s could exceed Cloudflare's free-tier KV read limits within a few hours. Votes now carry their data as KV list metadata, so `GET /api/state` reads them all in one `list()` call instead of one `get()` per vote, and both `/api/state` and `/api/pledge-state` are edge-cached for ~2s so simultaneous pollers share a read. If you're still seeing limit-exceeded notices with this in place, it's most likely genuinely high concurrent traffic — consider raising the KV plan, or lengthening the poll intervals in `courage-wall-production.html` (`pollState(...)` calls) further.
